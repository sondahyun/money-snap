-- Tripline ERDCloud용 단순화 SQL
-- 목적: ERDCloud에서 테이블/관계도를 안정적으로 그리기 위한 스키마
-- 주의: 이 파일은 PostgreSQL 실행용이 아니라 ERD 시각화용이다.
-- 차이점:
-- 1. enum 제거 -> varchar
-- 2. extension/default function 제거
-- 3. 복합 FK 제거
-- 4. jsonb/timestamptz 제거 -> text/timestamp
-- 5. check constraint / index 문 제거
